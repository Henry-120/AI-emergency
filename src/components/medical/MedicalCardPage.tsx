...
export function MedicalCardPage({ onBack }: { onBack: () => void }) {
  ...
  // 開啟頁面時從雲端載入最新醫療卡（離線時自動使用本機快取）。
  useEffect(() => {
    let active = true;
    fetchMedicalCard().then((c) => {
      if (active) setCard(c);
    });
    return () => {
      active = false;
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-[#020617] border-b border-white/5 shrink-0">
        ...
        {editing ? (
          <button
            onClick={handleSave}
            disabled={busy}
            className="px-3 py-1.5 rounded-lg bg-amber-500 text-black text-xs font-bold hover:bg-amber-400 disabled:opacity-50"
          >
            {busy ? "儲存中..." : "儲存"}
          </button>
        ) : (
          ...
          {editing ? (
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleSave} disabled={busy}
                className="w-full py-3 rounded-xl bg-amber-500 text-black font-bold text-sm hover:bg-amber-400 disabled:opacity-50">
                {busy ? "儲存中..." : "儲存醫療卡"}
              </button>
            </div>
          ) : (
