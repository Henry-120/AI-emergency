...
export function AppFooter({
  ...
  const toggleRecording = () => {
    if (!recognizerRef.current) {
      if (!speechSupported) {
        alert("此瀏覽器不支援語音辨識（SpeechRecognition）。請使用支援的瀏覽器或 HTTPS/localhost 測試。");
        return;
      }
      const rec = createSpeechRecognizer(
        (text: string, isFinal: boolean) => {
          // interim updates寫入輸入欄，final 結果則保留並顯示確認按鈕
          setInput(text);
          if (isFinal) {
      ...
      if (!rec.supported) {
        alert("此瀏覽器不支援語音辨識（SpeechRecognition）。");
        return;
      }
  ...
  return (
    <footer className="glass-panel p-4 safe-area-bottom">
      <div className="max-w-xl mx-auto">
        ...
        <form ref={formRef} onSubmit={(e) => { onSubmit(e); setFinalTranscript(""); }} className="relative flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <button
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 ${isRecording ? 'text-rose-400' : speechSupported ? 'text-slate-500' : 'text-slate-600/40'} active:text-amber-500`}
              aria-label={isRecording ? "停止語音輸入" : "開始語音輸入"}
              title={!speechSupported ? "此瀏覽器不支援語音辨識（需 HTTPS/localhost）" : isRecording ? "停止錄音" : "按此開始語音輸入，請先允許麥克風。"}
            >
              {/* 錄音中指示紅點 */}
              <span aria-hidden style={{ display: isRecording ? 'inline-block' : 'none', width: 10, height: 10, background: '#ff4d4f', borderRadius: '50%' }} />
              <i className={`fas fa-microphone`}></i>
            </button>
            {/* 無障礙：即時與最終辨識結果的 live region */}
            <div aria-live="polite" className="sr-only">
              {isRecording ? '錄音中' : finalTranscript ? `辨識完成：${finalTranscript}` : ''}
            </div>
          </div>
          {/* 當有 finalTranscript 時，顯示確認送出按鈕；按下會提交表單 */}
          {finalTranscript ? (
            <button
              type="button"
              onClick={() => {
                ...
                // 使用 formRef 提交，會觸發傳入的 onSubmit 並清除 finalTranscript
