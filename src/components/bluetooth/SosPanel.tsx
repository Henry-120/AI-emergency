  ...
  return (
    <div className="h-screen flex flex-col bg-bg overflow-hidden">
      <header className="safe-area-top border-b border-line bg-surface px-4 py-3 flex items-center gap-3 border-b border-line">
        ...
        <div className="text-ink font-bold">求救</div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between text-[12px] text-muted mb-2">
            <span>緊急度</span>
            <span className="text-critical-text font-semibold">{urgencyLevel} / 10</span>
          </div>
          <input
            type="range"
            min={1}
        ...
        <label className="flex items-center gap-2 bg-surface border border-line rounded-xl px-3 py-2 text-sm text-ink">
          <input type="checkbox" checked={isTrapped} onChange={(e) => setIsTrapped(e.target.checked)} />
          受困，出不去
        </label>

        <div>
          <div className="text-[12px] text-muted mb-2">行動能力</div>
        ...
        <div>
          <div className="text-[12px] text-muted mb-2">傷勢摘要（可用語音輸入）</div>
          <textarea
            value={injurySummary}
            onChange={(e) => setInjurySummary(e.target.value)}
            placeholder="例如：右腳受傷無法行走，頭部有撞擊"
            className="w-full h-20 bg-surface border border-line rounded-xl px-3 py-2 text-sm text-ink resize-none"
          />
        </div>

        <div>
          <div className="text-[12px] text-muted mb-2">需要什麼協助（可複選）</div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-surface border border-line rounded-xl px-3 py-2 text-sm text-ink"
          />
        </div>
        ...
        <label className="flex items-center gap-2 bg-surface border border-line rounded-xl px-3 py-2 text-sm text-ink">
          ...
          附上醫療卡摘要（血型、藥物過敏、慢性病）
          {!hasMedicalCard() && (
            <span className="text-[11px] text-muted">（尚未填寫醫療卡）</span>
          )}
        </label>

        {errorMsg && (
          <div className="px-3 py-2 bg-critical-soft text-critical-text text-[12px] rounded-xl border border-critical">
            {errorMsg}
          </div>
        )}

        {activeRecord && (
          <div className="px-3 py-2 bg-surface-2 text-accent text-[12px] rounded-xl border border-accent">
        ...
        <button
          onClick={handleSend}
          disabled={sending}
          className="w-full px-4 py-3 rounded-xl text-sm font-semibold bg-critical-soft text-critical-text border border-critical disabled:opacity-50"
        >
          {sending ? "送出中…" : "送出求救"}
        </button>
      </div>
