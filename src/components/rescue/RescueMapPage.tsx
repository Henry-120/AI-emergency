    ...
    <div className="h-[100dvh] flex flex-col bg-bg text-ink safe-area-top safe-area-bottom">
      ...
      <section className="max-h-[38vh] overflow-y-auto border-t border-line bg-bg p-3 space-y-2">
        ...
        {cases.length === 0 && sosCases.length === 0 && !loading && (
          <div className="py-6 text-center text-sm text-muted">附近目前沒有具 GPS 的待救援案件</div>
        )}
        {sosCases.map((item) => (
          <button
            key={item.msgId}
          ...
          >
            <div className="flex justify-between gap-3">
              ...
              <span className="text-critical-text">緊急度 {item.urgencyLevel}</span>
            </div>
            ...
            {item.locationDetails && (
              <div className="mt-1 text-xs text-muted">位置：{item.locationDetails}</div>
            )}
