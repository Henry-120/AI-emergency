...
async function resolveOfflineModelPath() {
  if (Capacitor.getPlatform() === "ios") {
    const installed = await OfflineModel.installBundledModel({
      fileName: OFFLINE_MODEL_FILE,
    });
...
function getLocalKnowledge(userInput: string, conversationText: string = userInput): DisasterAnalysis {
  ...
  // 5-1. 受傷主選單
    ...
    advice = "【受傷處置】1. 保持冷靜，深呼吸。2. 現在哪裡受傷了？請點選對應狀況：";
  ...
  // 5-2. 各個傷勢的自救處置
    ...
    options = ["已嘗試止血", "身邊沒有乾淨布料", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "骨折或脫臼") {
    advice = "【骨折評估】1. 絕對不要嘗試把骨頭推回去或拉直！ 2. 請保持該部位不動。請問是哪個部位疑似骨折或脫臼？";
    options = ["手臂或肩膀", "腿部或腳踝", "肋骨或胸部", "身邊沒有任何支撐物", "回首頁"];
    riskLevel = 8;
  }
    ...
    advice = "【心理安定】1. 現在很安全。請一起做：吸氣 4 秒、憋氣 2 秒、吐氣 6 秒。 2. 看看四周，找出 5 個我看到的物品、聽 3 個聲音。 3. 對自己說：『我正在等待救援，我會沒事的。』";
    options = ["情緒稍微平穩", "回首頁"];
    riskLevel = 4;
  }
  ...
  // 5-3. 延伸狀況 
    ...
    advice = "【原地待援】1. 請停止亂挖，避免再次塌陷。2. 每隔一段時間敲擊物品發出聲音。3. 記得保持體溫，蓋上身邊能蓋的衣服。";
    options = ["有呼吸困難", "有嚴重受傷", "回首頁"];
  }
  else if (userInput === "感覺意識模糊") {
    advice = "【緊急處置】1. 如果覺得快暈倒了，想辦法讓自己側躺（復甦姿勢），避免嘔吐物塞住呼吸道。2. 如果還有力氣，把手機設為警報模式或吹哨子。";
    options = ["已採復甦姿勢", "回首頁"];
    riskLevel = 10;
  }
  // 燒傷/吸入傷
    ...
    options = ["退到其他無火房間", "已逃至室外樓梯間", "已撤離至建築物外", "回首頁"];
    riskLevel = 10;
  }
    ...
    advice = "【持續監控】1. 做得好，保持冷靜，減少身體晃動來節省氧氣。2. 盯著手錶或手機，專心在每一次的呼吸。3. 如果感覺快暈倒，想辦法讓自己靠著牆壁坐穩，不要倒下阻塞氣管。";
    options = ["回首頁"];
    riskLevel = 10;
  }
  ...
  // 延伸選項：無法關閉開關
    ...
    advice = "【盡速撤離】1. 立即放棄關閉，維持現有電器狀態，直接開門。 2. 用濕毛巾或衣物掩住口鼻。 3. 壓低姿勢走樓梯撤離。";
  ...
  // 延伸選項：已關閉總開關
    ...
    advice = "【安全撤離】1. 做得好，維持現有電器狀態 2. 拿好避難包，用衣物護住頭部與口鼻。 3. 走樓梯撤離至戶外空曠處。樓梯間較通風，擴散較快，撤離是唯一安全路徑。";
    options = ["準備好出發撤離", "擔心撤離途中氣爆"];
    riskLevel = 7;
  }
  ...
  // 增加擔心撤離途中氣爆的選項
    ...
    advice = "【心理建設】1. 留在高濃度瓦斯室內遭遇氣爆與窒息的機率是 100%。2. 大樓樓梯間多有通風設計，濃度較低。 3. 撤離時用厚外套、防護墊或雙臂護住頭部與胸口。 4. 採取微蹲低姿勢。 5. 不要猶豫，迅速走樓梯向下，安全在於速度！";
    options = ["準備好出發撤離", "回首頁"];
    riskLevel = 9;
  }
  else if (userInput === "準備好出發撤離") {
    advice = "【行動指令】1. 保持冷靜，用衣物遮住口鼻，手護住頭部。2. 立刻出發走樓梯下樓！";
    options = ["回首頁"];
  }
  ...
  // 高樓層延伸
    ...
    advice = "【撤離提醒】1. 請攜帶緊急避難包，沿樓梯右側向下行進，將左側留給搜救人員。2. 抵達一樓後，迅速移動至最近的開放綠地或避難收容所。";
    options = ["已撤離至室外", "回首頁"];
  }
  ...
  // 受困：建築倒塌延伸
    ...
    advice = "【緊急自救】1. 若有大出血，利用衣物或皮帶在傷口上方進心端施加壓力。2. 骨折處請勿移動，找尋身邊硬物（如長木板、結實硬紙板）簡單固定。3. 保持體溫，防止休克。";
    options = ["已嘗試止血", "回首頁"];
    riskLevel = 10;
  }
  ...
  // 受困：水災延伸
    ...
    advice = "【水災危險】1. 拋棄沉重衣物。2. 將自己繫在大型漂浮物（如床墊、大型塑膠箱）上。3. 若在室內且水面逼近天花板，尋找是否有通往屋頂的出口或對外窗。";
    options = ["怎麼把漂浮物繫在身上？", "身邊無大型漂浮物", "屋頂沒有出口", "已往高層移動", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "水面漸緩或停滯") {
    advice = "【原地待援】1. 水勢雖緩，但嚴禁涉水離開！2. 積水極可能漏電，且水底常有銳器或掀開的人孔蓋。3. 請堅守目前的高處，保存體力，並利用手電筒（夜間）或懸掛鮮豔衣物（白天）向外發出求救信號。";
    options = ["已建立求救信號", "回首頁"];
    riskLevel = 10;
  }
  ...
  // 受困：火災延伸
    ...
    advice = "【對外求生】1. 迅速爬至窗邊或陽台。2. 若是高樓層，請靠在窗邊呼吸新鮮空氣，大聲呼救並揮舞明顯衣物，讓消防員能第一時間看到您，切勿盲目跳樓！3. 若是低樓層且窗外安全，才可考慮攀爬逃生。";
    options = ["火場濃煙呼吸困難", "正在等待救援", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "身上衣物著火") {
    advice = "【停、趴、滾】1. 不要慌張奔跑，風會讓火變大！2. 停：立刻停在原地。3. 趴：馬上雙手掩住臉部趴在地上。4. 滾：左右來回翻滾，直到火勢撲滅。嚴禁用手拍打火勢。5. 若解決且無大礙，請確認目前處境。";
    options = ["衣物黏在皮膚上", "傷口出現水泡", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "被火包圍無法移動") {
    advice = "【最後防護】1. 若無路可退，請趴在離火最遠的角落。2. 用厚重衣物或棉被（若能弄濕更好）將自己全身覆蓋。3. 務必將臉部朝下貼緊地面，專注於淺層呼吸，等待消防隊射水救援。";
    options = ["火場濃煙呼吸困難", "焦慮過度", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "退到其他無火房間") {
    advice = "【防線建立】1. 立刻關上這扇門，把火擋在外面！2. 尋找毛巾或衣物塞住門縫，並移往有對外窗的位置求救。";
    options = ["門把發燙", "煙霧進入室內", "回首頁"];
    riskLevel = 10;
  }
  ...
  // 出口受阻：建築倒塌延伸
    ...
    advice = "【謹慎移動】1. 緩慢向出口移動，注意頭部上方瓦礫是否鬆動。2. 每移動一段距離請停下聽有無異常聲響。3. 若出口狹窄，確保身體能通過再前進，避免再次受困。";
    options = ["已安全離開", "回首頁"];
  }
  ...
  // 延伸確認：已建立求救信號
    ...
    advice = "【保存體力】1. 做得好，現在請保持冷靜，深呼吸穩定情緒。2. 每隔一段時間再次敲擊信號。3. 若手機還有電力，將其設為省電模式，並每小時檢查一次訊息。";
    options = ["回首頁"];
  }
  // 出口受阻：水災延伸
  else if (userInput === "已往高層移動") {
    advice = "【高處待援】1. 確認手機訊號與電量。2. 若夜間請規律閃爍手電筒發出 SOS 訊號。3. 切勿因水面暫時退去就嘗試下樓，等待官方確認安全。";
    options = ["發出訊號中", "回首頁"];
  }

  // 出口受阻：火災延伸
  else if (userInput === "門把發燙") {
    advice = "【嚴禁開門】1. 這代表門外已有劇烈火煙！立即關閉所有內部門窗減緩火勢蔓延。2. 退往離火源最遠且有對外窗的房間，用衣物塞住所有門縫，等待消防隊。";
    options = ["已進入避難房間", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "煙霧進入室內") {
    advice = "【防煙應變】1. 立即採取低姿勢（地表 30 公分內仍有剩餘新鮮空氣）。2. 尋找毛巾或衣物（若有水弄濕更好）徹底塞住所有門縫。3. 若有對外窗，稍微打開窗戶縫隙呼吸，並向外揮舞明顯色彩衣物。4. 絕對不要躲在浴室！";
    options = ["已採低姿勢", "火場濃煙呼吸困難", "回首頁"];
    riskLevel = 10;
  }

  // 受傷 : 大量出血延伸
  else if (userInput === "身邊沒有乾淨布料") {
    advice = "【極限止血】1. 救命第一，現在不要管感染問題！ 2. 立刻脫下自己或傷者的衣服（即使髒污也沒關係），摺疊後對準傷口用力重壓！ 3. 若連衣物都沒有，請直接用雙手『掌根』或『手指』對準出血點死命壓住。 4. 壓住後『絕對不要』鬆手檢查有沒有流血，持續重壓直到救援抵達。";
    options = ["已嘗試止血", "回首頁"];
    riskLevel = 10;
  }
  // 受傷 : 骨折、脫臼延伸
  else if (userInput === "手臂或肩膀") {
    advice = "【上肢固定】1. 盡量維持受傷時最不痛的姿勢。 2. 找木板、硬紙板或捲起的雜誌、雨傘撐住手臂。 3. 找布條將受傷手臂與支撐物綁在一起，並『輕輕綁在自己的胸口』，利用軀幹減少晃動。";
    options = ["已完成固定", "身邊沒有任何支撐物", "回首頁"];
    riskLevel = 7;
  }
  else if (userInput === "腿部或腳踝") {
    advice = "【下肢固定】1. 絕對不能站立或走路！ 2. 在受傷的腿旁放上長木板、長柄傘或任何堅硬長條物。 3. 用布條或皮帶固定，必須『同時綁住受傷部位的上下兩個關節』。 4. 觀察腳趾：若發紫或冰冷代表綁太緊，請立刻放鬆。";
    options = ["已完成固定", "身邊沒有任何支撐物", "回首頁"];
    riskLevel = 8;
  }
  else if (userInput === "肋骨或胸部") {
    advice = "【軀幹防護】1. 絕對不要用繃帶把胸部纏死，會導致無法呼吸！ 2. 拿一件厚外套、小枕頭或背包，讓傷者『抱在』受傷的肋骨那一側，減輕呼吸與咳嗽時的劇痛。 3. 採取半坐臥姿勢休息，不要完全躺平。";
    // 肋骨骨折最怕刺穿肺部，需加入呼吸困難的監測
    options = ["呼吸越來越困難", "回首頁"]; 
    riskLevel = 9;
  }
  // 缺乏支撐物的極限應變：自體固定法 (Anatomical splinting)
  else if (userInput === "身邊沒有任何支撐物") {
    advice = "【自體固定法】1. 不要慌，請把『傷者自己未受傷的身體』當作夾板！ 2. 【手臂受傷】：直接將受傷的手臂平貼在胸口，用衣服、皮帶或包包的背帶，把手臂跟軀幹死死綁在一起。 3. 【腿部受傷】：將『受傷的腿』與『健康的腿』併攏，中間夾一點衣物當緩衝，接著用鞋帶或撕開的衣物，將兩條腿綁在一起固定。";
    options = ["已完成自體固定", "回首頁"];
    riskLevel = 8;
  }
  // 肋骨骨折引發氣胸/血胸的極端情況
  else if (userInput === "呼吸越來越困難") {
    advice = "【氣胸警訊】1. 斷裂的肋骨可能已傷及肺部。 2. 絕對不可以躺平！請協助傷者採取『半坐臥姿勢』（背部墊高約 45 度）。 3. 讓傷者身體向『受傷的那一側』稍微傾斜，這樣能讓健康那一側的肺部有最大空間擴張呼吸。 4. 嚴密監控意識。";
    options = ["我確信我能做到", "回首頁"];
    riskLevel = 10;
  }
  ...
  // 安全確認
    ...
    advice = "【室外避難】1. 做得好！請前往空曠處（如公園、學校操場），遠離高大建築物、外牆磁磚、圍牆及電線桿。2. 持續注意餘震，並嘗試與家人報平安。";
    options = ["回首頁"];
  ...
  // 8. 深度延伸指令 (第二層)
    ...
    advice = "【固定漂浮物】1. 利用皮帶、背包帶或撕開的長條衣物。2. 將繩索穿過漂浮物穩固處，綁在自己的『腋下與胸部』之間。3. 絕對不要綁在腰部（會導致頭下腳上）或脖子（會勒窒息），確保頭部能時刻浮出水面。";
    options = ["已完成固定", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "身邊無大型漂浮物") {
    advice = "【替代漂浮物】1. 尋找空寶特瓶（務必旋緊瓶蓋）、密封的塑膠袋、保鮮盒或防潑水背包。2. 將這些充滿空氣的物品塞進外套內部或衣服內，拉上拉鍊或將下擺紮進褲子裡，這能為身體提供重要的臨時浮力。";
    options = ["已製作臨時浮力衣", "回首頁"];
    riskLevel = 10;
  }
  else if (userInput === "屋頂沒有出口") {
    advice = "【防溺應變】1. 絕對不要逃進沒有逃生口的密閉閣樓！ 2. 若水面逼近天花板，立刻尋找對外窗戶，用重物（如椅子、滅火器）擊碎窗戶四角逃生。3. 若無法破窗，請盡可能抬高下巴，讓口鼻貼近天花板剩下的最後空氣層，規律敲擊天花板或牆壁求救。";
  ...
  else if (userInput === "已逃至室外樓梯間") {
    advice = "【樓梯間撤離】1. 隨手關上身後的門！（這能把火與煙擋在屋內，拯救整棟大樓）。 2. 絕對不可以搭電梯。 3. 觀察樓梯間是否有煙？如果沒有，請立刻往下走至一樓戶外。";
    options = ["樓梯間有濃煙", "已撤離至建築物外", "回首頁"];
    riskLevel = 7;
  }
  // 處理樓梯間有煙的致命狀況
  else if (userInput === "樓梯間有濃煙") {
    advice = "【致命危險！停止往下】1. 絕對不可以強行穿越濃煙！煙每秒向上竄升 3-5 公尺，你跑不贏它。 2. 不要往頂樓跑（屋頂可能上鎖，且煙會聚集在頂部）。 3. 請立刻退回剛才的樓層或最近的無煙樓層，隨手關門。 4. 尋找有對外窗的房間「關門避難」。";
    // 串接回原有的就地避難邏輯
    options = ["退到其他無火房間", "回首頁"]; 
    riskLevel = 10;
  }
  // 完全脫險的分支
  else if (userInput === "已撤離至建築物外") {
    advice = "【戶外安全區】1. 做得好！您已安全。請退到建築物外圍空曠處，小心上方玻璃或磁磚掉落。 2. 絕對不要為了拿財物或尋找寵物重返火場！ 3. 抵達安全處後，請盡速向消防人員報到，告知起火點以及是否還有家人受困。";
    options = ["回首頁"];
    riskLevel = 2; // 風險解除
  }
...
請完全依照以下格式輸出（必須包含大括號）：
{
  ...
  try {
    ...
    // 裝置端模型是 iOS 原生外掛，瀏覽器沒有它。
    // 在 web 上直接走靜態降級方案，不要試著載入一個不存在的原生模組——
    // 靜態 import 會讓它壞掉的 chunk 在每次開啟頁面時丟出 ReferenceError。
    if (!Capacitor.isNativePlatform()) {
      throw new Error("裝置端模型僅在 App 內可用，網頁版改用靜態降級方案");
    }

    // 1. 載入模型並獲取 LlamaContext 實例（僅原生平台）
    ...
    console.error("LLM 解析失敗，啟動強化版靜態降級方案", error);
